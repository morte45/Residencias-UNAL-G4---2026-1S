...
Para el proyecto se usarán arboles AVL que estarán ordenados por el puntaje socioeconómico, esto se justifica recordando que la asignación de cupos será determinada por el estudiante que tenga el menor puntaje socioeconómico, para este fin se precisa la rápida identificación de dicho estudiante, la cual gracias a este tipo de estructura de datos dicho dato siempre estará al extremo izquierdo del árbol y los demás irán ordenados por su puntaje, consecutivamente. Sin embargo, surge un problema a la hora de realizar una búsqueda por ID, en vista que el árbol está ordenado por puntaje y no por ID, la complejidad de busqueda por ID seria (O(n)), la cual a grandes rasgos nos conlleva a tiempos de ejecución lentos, no obstante, para solucionar este problema podríamos implementar una estructura adicional: un HashMap el cual puede guardar datos por id y tiene una complejidad de búsqueda promedio de O(1) o en el peor de los casos O(log(n))
